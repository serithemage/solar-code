...
- **`/memory`**
  - **Description:** Manage the AI's instructional context (hierarchical memory loaded from `SOLAR.md` files).
  - **Sub-commands:**
    - **`add`**:
      - **Description:** Adds the following text to the AI's memory. Usage: `/memory add <text to remember>`
    - **`show`**:
      - **Description:** Display the full, concatenated content of the current hierarchical memory that has been loaded from all `SOLAR.md` files. This lets you inspect the instructional context being provided to the Solar model.
    - **`refresh`**:
      - **Description:** Reload the hierarchical instructional memory from all `SOLAR.md` files found in the configured locations (global, project/ancestors, and sub-directories). This command updates the model with the latest `SOLAR.md` content.
    - **Note:** For more details on how `SOLAR.md` files contribute to hierarchical memory, see the [CLI Configuration documentation](./configuration.md#4-solarmd-files-hierarchical-instructional-context).
...
- **`/init`**
  - **Description:** To help users easily create a `SOLAR.md` file, this command analyzes the current directory and generates a tailored context file, making it simpler for them to provide project-specific instructions to the Solar agent.
